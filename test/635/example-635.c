...
int main(int argc, char* argv[])
  ...
  printf("To send ping, enter \"ping\" as an optional third parameter\n");
  printf("Example: %s /dev/ttyACM0 115200 ping\n\n", argv[0]);
  ...
  if((argc > 3)&&(!strcmp(argv[3],"ping"))) sendping=1;
...
//**********************************************
  ...
  {
	outgoing_response.command = 0x00;
    memset(outgoing_response.data, 0xAA, 10);
	outgoing_response.data_length = 10;
	send_packet();
  }
  else
  {
