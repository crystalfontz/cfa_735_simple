 ...
 */
/*
 * These are the STM32 USB interrupts that can be enabled if the
 * application wants to be called back on USB state changes
 */
...
u32 STM32_PCD_OTG_ISR_Handler (void)
{
  ...
  if (USBD_FS_IsDeviceMode()) /* ensure that we are in device mode */
  {
   ...
   /*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/    
    ...
    {
        /* read the interrupts */
        uint32_t otgreg = OTGD_FS_ReadOtgItr();
        /* handle the interrupts */
        if (otgreg & 0x4) /* disconnect */
        {
            /* TODO: move this up to the application so it can happen in application space and use real timings*/
            bDeviceState = UNCONNECTED;
            PowerOff();
            Virtual_Com_Port_init();
            retval |= 1;
        }
        /* clear the interrupts on a Connectivity Line Device */
        *((uint32_t*)0x50000004) = otgreg;
    }
#endif /* INTR_GOTGR */  
